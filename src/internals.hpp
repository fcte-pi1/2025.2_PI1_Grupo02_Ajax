///
/// internals.hpp
/// Encapsula toda a configuração de periféricos e GPIOs internas ao microcontrolador
///
...
namespace internals {
  ...
  inline
  auto initialize_serial( ) -> void {
    Serial.begin(constants::BAUD_RATE);
    delay(1000);
    Serial.println("[INFO] Comunicacao UART inicializada com baud rate de 115200.");
  }

  /// @brief Configura o ponto de acesso de WIFI roteado pela ESP32.
  /// @return Se a configuração funcionou ou não.
  inline
  auto initialize_wifi( ) -> bool {
    if ( !WiFi.softAPConfig(constants::STATIC_IP_ADDR, constants::STATIC_IP_ADDR, constants::GATEWAY) ) {
